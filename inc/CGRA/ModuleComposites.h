...
{
    public:
        MemPort(std::string, int num_connections, unsigned size = DEFAULT_SIZE);
        virtual std::string GenericName();
        virtual ~MemPort();
    private:
        unsigned num_connections; // number of muxed inputs to this memory unit
};

class IOPort : public Module
{
    public:
        IOPort(std::string, int num_inputs, unsigned size = DEFAULT_SIZE);
        virtual std::string GenericName();
        virtual ~IOPort();
    private:
        unsigned num_inputs; // number of muxed inputs to this IO
};

class SimpleFU : public Module
{
    public:
        SimpleFU(std::string, CGRABlockType BlockType = STANDARD_NOBYPASS);
        virtual std::string GenericName();
        virtual ~SimpleFU();
